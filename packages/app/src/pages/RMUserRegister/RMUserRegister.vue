...
const btnClickState = ref<boolean>(false)
const confirmDrawerOpen = ref<boolean>(false)
const registerBtnLabel = computed(() => {
  return btnClickState.value ? '登録' : '登録確認'
})
const cancelBtnLabel = computed(() => {
  return btnClickState.value ? '修正' : 'キャンセル'
})

const registerConfirm = () => {
  if (!btnClickState.value) {
    btnClickState.value = true
    // ドロワーオープン
    confirmDrawerOpen.value = true
    return
  }
}
</script>
