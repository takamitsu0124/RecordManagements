...
const userSkillsByUserId = computed(() => {
	return new Map(
		skillStore.state.currentUserSkills.map((userSkill) => [userSkill.userId, userSkill])
	)
})
...
const memberSkillSummaries = computed<GuildMemberSkillSummary[]>(() => {
	return approvedMembers.value.map((member) => {
		...
		const unlockRate = masterCount.value > 0 ? (ownedCount / masterCount.value) * 100 : 0
...
const averageUnlockRate = computed(() => {
	...
	return (
		memberSkillSummaries.value.reduce((total, member) => total + member.unlockRate, 0) /
		memberSkillSummaries.value.length
	)
})

const guildSkillRows = computed<GuildSkillRow[]>(() => {
	const memberMap = new Map(approvedMembers.value.map((member) => [member.uid, member]))
...
const normalizedSkillNameFilter = computed(() =>
	String(skillTableFilters.value.skillName.value || '').trim().toLowerCase()
)

const selectedAttrFilter = computed(() => skillTableFilters.value.attr.value || '')
const selectedTypeFilter = computed(() => skillTableFilters.value.type.value || '')
const selectedLevelFilter = computed(() => Number(skillTableFilters.value.level.value || 0))
...
const attrOptions = computed(() => {
	return [...new Set(skillStore.state.masterData.map((skill) => skill.attr).filter(Boolean))]
		.sort((a, b) => a.localeCompare(b, 'ja'))
		.map((attr) => ({ label: attr, value: attr }))
})

const typeOptions = computed(() => {
	return [...new Set(skillStore.state.masterData.map((skill) => skill.type).filter(Boolean))]
		.sort((a, b) => a.localeCompare(b, 'ja'))
...
const summaryItems = computed(() => [
	...
	},
	{
		label: '現在の表示モード',
		value:
			isEditMode.value && canManageGuildMembers.value ? '管理モード' : '閲覧モード',
							...
							<div class="guild-operations">
								<div class="guild-operations__item">
									...
									<p class="guild-operations__text">
										スキル名は 1 文字入力するごとに即時反映されます。属性・種別・熟練度も同時に絞り込めます。
									</p>
								</div>
								<div class="guild-operations__item">
							...
							<div v-else class="guild-skill-overview-list">
								...
								>
									<div class="guild-skill-overview-item__head">
										<div>
											<div class="guild-skill-overview-item__name">{{ member.displayName }}</div>
							...
							<template v-else>
								<div class="guild-search-table-shell rm-mobile-scroll">
									...
									>
										...
										<Column field="unlockRateText" header="解放率" style="width: 120px" />
								...
								<div class="guild-skill-mobile-list">
									...
									>
										<div class="guild-skill-mobile-item__head">
											<div>
												<div class="guild-skill-mobile-item__skill">{{ row.skillName }}</div>
