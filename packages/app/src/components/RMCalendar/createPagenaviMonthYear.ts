class CreatePagenaviMonthYear {
   ...
   * ページナビの年月を生成する
   */
  public createPagenaviEnMonthYear = (
    yearValue: number,
    monthValue: number
  ) => {
  ...
  public createPagenaviJaMonthYear = (
    yearValue: number,
    monthValue: number
  ) => {
    return monthValue >= 0 && monthValue <= 11
      ? `${yearValue}年 ${monthValue + 1}月`
      : ''
  }
  public createPagenaviMonthYearChoose = (
    language: string,
    yearValue: number,
    monthValue: number
  ) => {
