...
const cloneSkillRecord = (
  payload?: Partial<SkillRecord> | null
): SkillRecord => {
  ...
  const rawUrls =
    payload.imageUrls.length > 0
      ...
      : legacyImageKeys.flatMap((key) =>
          Array.isArray(payload.skillRecord[key])
            ? payload.skillRecord[key]
            : []
        )
  ...
  await useSpinner(async () => {
    try {
      ...
      await syncUserDocuments({
        uid: props.userId,
        operation: props.includeProfile
          ? 'user-workspace-profile-save'
          : 'user-workspace-assets-save',
        appUser:
          props.includeProfile && nextAppUser
            ? {
                mode: appUserExists.value ? 'merge' : 'insert',
                payload: appUserExists.value
                  ? {
            ...
            : null,
        legacyUser: {
          mode: legacyUserExists.value
            ? 'merge'
            : props.includeProfile
            ? 'insert'
            : 'merge',
          payload: legacyUserExists.value
            ? {
                ...(props.includeProfile
                  ? {
      ...
      if (props.includeProfile && nextAppUser && !appUserExists.value) {
